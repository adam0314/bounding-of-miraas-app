...
func set_nodes_and_parameters(input_item_object):
	...
	item_texture_node.modulate = Global.get_color_for_sign(item_object.type)
	type_label_node.text = get_text_for_item_type()
	effect_label_node.text = item_object.effect
	pass

func get_color_for_tex() -> Color:
	match item_object.type:
